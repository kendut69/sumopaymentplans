...
class SUMO_PP_Stripe_API_Request {
	...
	public static function init( SUMO_PP_Stripe_Gateway $stripe_gateway ) {
		...
		if ( ! class_exists( '\Stripe\Stripe' ) ) {
			throw new Exception( __( 'Cannot connect to Stripe.', 'sumopaymentplans' ) ) ;
		}
	...
	public static function get_localized_messages() {
		return apply_filters( 'sumopaymentplans_stripe_localized_messages', array(
			'invalid_number'           => __( 'The card number is not a valid credit card number.', 'sumopaymentplans' ),
			'invalid_expiry_month'     => __( 'The card\'s expiration month is invalid.', 'sumopaymentplans' ),
			'invalid_expiry_year'      => __( 'The card\'s expiration year is invalid.', 'sumopaymentplans' ),
			'invalid_cvc'              => __( 'The card\'s security code is invalid.', 'sumopaymentplans' ),
			'incorrect_number'         => __( 'The card number is incorrect.', 'sumopaymentplans' ),
			'incomplete_number'        => __( 'The card number is incomplete.', 'sumopaymentplans' ),
			'incomplete_cvc'           => __( 'The card\'s security code is incomplete.', 'sumopaymentplans' ),
			'incomplete_expiry'        => __( 'The card\'s expiration date is incomplete.', 'sumopaymentplans' ),
			'expired_card'             => __( 'The card has expired.', 'sumopaymentplans' ),
			'incorrect_cvc'            => __( 'The card\'s security code is incorrect.', 'sumopaymentplans' ),
			'incorrect_zip'            => __( 'The card\'s zip code failed validation.', 'sumopaymentplans' ),
			'invalid_expiry_year_past' => __( 'The card\'s expiration year is in the past', 'sumopaymentplans' ),
			'card_declined'            => __( 'The card was declined.', 'sumopaymentplans' ),
			'missing'                  => __( 'There is no card on a customer that is being charged.', 'sumopaymentplans' ),
			'processing_error'         => __( 'An error occurred while processing the card.', 'sumopaymentplans' ),
			'invalid_request_error'    => __( 'Unable to process this payment, please try again or use alternative method.', 'sumopaymentplans' ),
			'email_invalid'            => __( 'Invalid email address, please correct and try again.', 'sumopaymentplans' ),
				) ) ;
	}
	...
	public static function prepare_customer_details( $params, $exclude_params = array() ) {
		...
		if ( empty( $params[ 'description' ] ) ) {
			...
			$params[ 'description' ] = sprintf( __( '%1$s - Name: %2$s', 'sumopaymentplans' ), wp_specialchars_decode( get_bloginfo( 'name' ), ENT_QUOTES ), $params[ 'name' ] ) ;
		}
	...
	public static function get_last_error_message( $localized = true ) {
		...
		if ( $localized ) {
			...
			} elseif ( isset( $localized_messages[ $type ] ) ) {
					$error_message = $localized_messages[ $type ] ;
			}
		}

		if ( empty( $error_message ) ) {
			$error_message = __( 'An error occurred while processing the card', 'sumopaymentplans' ) ;
		}
