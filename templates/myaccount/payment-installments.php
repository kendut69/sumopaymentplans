<?php
/**
 * My Payments > View Payment > Installments
 * 
 * Shows the details of a particular payment installments on the account page.
 *
 * This template can be overridden by copying it to yourtheme/sumopaymentplans/myaccount/payment-installments.php.
 */
defined( 'ABSPATH' ) || exit ;

do_action( 'sumopaymentplans_before_my_account_payment_installments_table', $payment, $initial_payment_order ) ;
?>
<table class="my_account_sumo_payment_installments my_account_orders account-orders-table woocommerce-orders-table woocommerce-MyAccount-orders shop_table shop_table_responsive">
	<thead>
		<tr>
			<?php foreach ( $columns as $column_key => $column_name ) : ?>
				<th class="sumo-<?php echo esc_attr( $column_key ) ; ?> woocommerce-orders-table__header woocommerce-orders-table__header-<?php echo esc_attr( $column_key ) ; ?>"><span class="nobr"><?php echo esc_html( $column_name ) ; ?></span></th>
			<?php endforeach ; ?>
		</tr>
	</thead>
	<tbody>
		<?php do_action( 'sumopaymentplans_my_account_payment_installments_table_row_start', $payment, $initial_payment_order ) ; ?>
		<?php
		if ( 'pay-in-deposit' === $payment->get_payment_type() ) {
			...
			?>
			<tr class="woocommerce-orders-table__row woocommerce-orders-table__row--sumo-payment-installment">
				<?php foreach ( $columns as $column_key => $column_name ) : ?>
					<td class="sumo-<?php echo esc_attr( $column_key ) ; ?> woocommerce-orders-table__cell woocommerce-orders-table__cell-<?php echo esc_attr( $column_key ) ; ?>" data-title="<?php echo esc_attr( $column_name ) ; ?>">
						<?php
						switch ( $column_key ) {
							...
							case 'installment-order-number':
								...
								break ;
							default:
								if ( has_action( 'sumopaymentplans_my_account_deposit_payment_installments_column_' . $column_key ) ) {
									do_action( 'sumopaymentplans_my_account_deposit_payment_installments_column_' . $column_key, $payment ) ;
								}
						}
						?>
					</td>
							...
							<td class="sumo-<?php echo esc_attr( $column_key ) ; ?> woocommerce-orders-table__cell woocommerce-orders-table__cell-<?php echo esc_attr( $column_key ) ; ?>" data-title="<?php echo esc_attr( $column_name ) ; ?>">
								...
								switch ( $column_key ) {
									...
									case 'installment-order-number':
										...
										break ;
									default:
										if ( has_action( 'sumopaymentplans_my_account_payment_plan_installments_column_' . $column_key ) ) {
											do_action( 'sumopaymentplans_my_account_payment_plan_installments_column_' . $column_key, $payment, $installment, $scheduled_installment ) ;
										}
								}
								?>
							</td>
		...
		?>
		<?php do_action( 'sumopaymentplans_my_account_payment_installments_table_row_end', $payment, $initial_payment_order ) ; ?>
	</tbody>
</table>
<?php
do_action( 'sumopaymentplans_after_my_account_payment_installments_table', $payment, $initial_payment_order ) ;
