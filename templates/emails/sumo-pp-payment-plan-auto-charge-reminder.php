...
<?php if ( $payment_order->has_status( 'pending' ) ) : ?>
	...
	<p>
		<?php
		/* translators: 1: product title with installment number 2: payment number 3: next payment date */
		printf( wp_kses_post( __( 'Hi, <br>This is to remind you that %1$s for your payment #%2$s will be automatically charged on <b>%3$s</b> because you have already preapproved for automatic charging. <br>Kindly make sure you have sufficient funds in your account.', 'sumopaymentplans' ) ), wp_kses_post( $product_title_with_installment ), esc_html( $payment->get_payment_number() ), esc_html( _sumo_pp_get_date_to_display( $payment->get_prop( 'next_payment_date' ), 'email' ) ) ) ;
		?>
	</p>
